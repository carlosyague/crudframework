...
public class ArtifactInstaller {
    ...
    public ArtifactInstaller(MavenProject project, String pojoName,
            ...
            String destinationSampleDataFile, boolean genericCore) {
    ...
    public void execute() {
        ...
        // install dao and manager if jar (modular/core) or war w/o parent
        // (basic)
        ...
        if (project.getPackaging().equalsIgnoreCase("war")) {
            ...
            if ("jsf".equalsIgnoreCase(webFramework)) {
                ...
                copyGeneratedObjects(sourceDirectory + "/src/main/resources",
                        destinationDirectory + "/src/main/resources",
                        "**/model/*.xml");
                copyGeneratedObjects(sourceDirectory + "/src/main/resources",
                        destinationDirectory + "/src/main/resources",
                        "**/webapp/action/*.xml");
     ...
     * directory based on the pattenr.
     * 
    ...
    private String getPathToApplicationContextDao() {
        if (project.getPackaging().equalsIgnoreCase("war")) {
            return "/src/main/webapp/WEB-INF/spring/applicationContext.xml";
        } else { // if (project.getPackaging().equalsIgnoreCase("jar")) {
            return "/src/main/resources/spring/applicationContext-dao.xml";
        }
    }

    private String getPathToApplicationContextService() {
        if (project.getPackaging().equalsIgnoreCase("war")) {
            return "/src/main/webapp/WEB-INF/spring/applicationContext.xml";
        } else { // if (project.getPackaging().equalsIgnoreCase("jar")) {
            return "/src/main/resources/spring/applicationContext-service.xml";
        }
    }
    ...
    private void installSampleData() {
        ...
        createLoadFileTask(
                "src/test/resources/" + pojoName + "-sample-data.xml",
                "sample.data").execute();
        File existingFile = new File(destinationSampleDataFile);

        parseXMLFile(existingFile, null, "</dataset>", "sample.data");

        log("Updated /src/test/resources/sample-data.xml");
    }

    private void installDaoAndManagerBeanDefinitions() {
        createLoadFileTask("src/main/resources/" + pojoName + "Dao-bean.xml",
                "dao.context.file").execute();
        File generatedFile = new File(destinationDirectory
                + getPathToApplicationContextDao());

        parseXMLFile(generatedFile, pojoName + "Dao",
                "<!-- Add new DAOs here -->", "dao.context.file");

        log("Updated " + getPathToApplicationContextDao()
                + " with a new DAO definition.");

        createLoadFileTask(
                "src/main/resources/" + pojoName + "Manager-bean.xml",
                "mgr.context.file").execute();
        generatedFile = new File(destinationDirectory
                + getPathToApplicationContextService());

        parseXMLFile(generatedFile, pojoName + "Manager",
                "<!-- Add new Managers here -->", "mgr.context.file");

        log("Updated " + getPathToApplicationContextService()
                + " with a new Manager definition.");
    }

    private void installiBATISFiles() {
        if (project.getProperties().getProperty("dao.framework").equals(
                "ibatis")) {
            log("Installing iBATIS SQL Maps...");
            createLoadFileTask(
                    "src/main/resources/" + pojoName + "-sql-map-config.xml",
                    "sql.map.config").execute();
            File sqlMapConfig = new File(destinationDirectory
                    + "/src/main/resources/sql-map-config.xml");
            parseXMLFile(sqlMapConfig, null, "</sqlMapConfig>",
                    "sql.map.config");

            File sqlMapsDir = new File(destinationDirectory
    ...
    private void installGenericBeanDefinitions() {
        createLoadFileTask(
                "src/main/resources/" + pojoName + "-generic-beans.xml",
                "context.file").execute();
        File generatedFile = new File(destinationDirectory
                + getPathToApplicationContext());

        parseXMLFile(generatedFile, pojoName + "Manager",
                "<!-- Add new Managers here -->", "context.file");
    }

    private void installJSFNavigationAndBeans() {
        createLoadFileTask(
                "src/main/webapp/WEB-INF/" + pojoName + "-navigation.xml",
                "navigation.rules").execute();
        File generatedFile = new File(destinationDirectory
                + "/src/main/webapp/WEB-INF/faces-config.xml");
        parseXMLFile(generatedFile, pojoName + "-nav",
                "<!-- Add additional rules here -->", "navigation.rules");

        log("Updated /src/main/webapp/WEB-INF/faces-config.xml with navigation-rules.");

        createLoadFileTask(
                "src/main/webapp/WEB-INF/" + pojoName + "-managed-beans.xml",
                "managed.beans").execute();
        generatedFile = new File(destinationDirectory
                + "/src/main/webapp/WEB-INF/faces-config.xml");
        parseXMLFile(generatedFile, pojoName + "-beans",
                "<!-- Add additional beans here -->", "managed.beans");

        log("Updated /src/main/webapp/WEB-INF/faces-config.xml with managed-beans.");
    }

    private void installSpringControllerBeanDefinitions() {
        createLoadFileTask(
                "src/main/webapp/WEB-INF/" + pojoName + "-beans.xml",
                "dispatcher.servlet").execute();
        File generatedFile = new File(destinationDirectory
    ...
    private void installSpringValidation() {
        createLoadFileTask(
                "src/main/webapp/WEB-INF/" + pojoName + "-validation.xml",
                "struts.validation").execute();
        File generatedFile = new File(destinationDirectory
                + "/src/main/webapp/WEB-INF/validation.xml");

        parseXMLFile(generatedFile, pojoName, "    </formset>",
                "struts.validation");
    }

    private void installStrutsActionDefinitions() {
        createLoadFileTask("src/main/resources/" + pojoName + "-struts.xml",
                "struts.file").execute();
        File existingFile = new File(destinationDirectory
                + "/src/main/resources/struts.xml");

        parseXMLFile(existingFile, pojoName + "Action",
                "<!-- Add additional actions here -->", "struts.file");
    }
    ...
    private String groupIdToPath() {
        String classpath = this.project.getGroupId();

        return classpath.replace('.', '/');
    }

    private String getApplicationResourcesFile() {
        String result = "/src/main/resources/";

        // result += "ApplicationResources.properties";
        result += groupIdToPath() + "/messages.properties";
    ...
    private void installInternationalizationKeys() {
        createLoadFileTask(
                "src/main/resources/" + pojoName
                        + "-ApplicationResources.properties", "i18n.file")
                .execute();
        File existingFile = new File(destinationDirectory
                + getApplicationResourcesFile());
