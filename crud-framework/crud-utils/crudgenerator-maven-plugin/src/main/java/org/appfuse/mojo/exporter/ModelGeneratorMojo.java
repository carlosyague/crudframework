...
/**
 * Generates Java classes from existing database tables. If you want to
 * customize the reverse engineering strategy, you can modify the default <a
 * href="http://tinyurl.com/3qqgcl">hibernate.reveng.xml</a> and put it in
 * src/test/resources. You can also override the location by specifying the
 * "revengfile" property in the &lt;configuration&gt;. For example:
 * 
 * <pre>
 *  &lt;configuration&gt;
 *    &lt;componentProperties&gt;
 ...
 * 
 * <p>
 * <b>Oracle Users</b>: If you create a custom hibernate.reveng.xml, make sure
 * to capitalize the table names in &lt;schema-selection
 * match-schema="MY_SCHEMA_NAME"/&gt;. You'll also need to add the following
 * line to your jdbc.properties file:
 * </p>
 * 
 * <pre>
...
public class ModelGeneratorMojo extends HibernateExporterMojo {
    ...
    private String  commonCorePackage = null;
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        getComponentProperties().put(
                ...
                (sourceDirectory != null) ? sourceDirectory
                        : "${basedir}/target/appfuse/generated-sources");
        ...
        if (!existingConfig.exists()) {
            InputStream in = this.getClass().getResourceAsStream(
                    "/appfuse/model/hibernate.reveng.ftl");
            ...
            try {
                ...
                FileUtils.writeStringToFile(existingConfig, configFile
                        .toString());
        ...
        if (getComponentProperty("packagename") == null) {
            getComponentProperties().put("packagename",
                    getProject().getGroupId() + ".model");
        }
        ...
        if (getComponentProperty("configurationfile") == null) {
            ...
            if (jdbcProperties.exists()) {
                if (getComponentProperty("propertyfile") == null) {
                    getComponentProperties().put("propertyfile",
                            jdbcProperties.getPath());
                    getLog().debug(
                            "Set propertyfile to '" + jdbcProperties.getPath()
                                    + "'");
                }
            } else {
                throw new MojoFailureException(
                        "Failed to find jdbc.properties in classpath.");
            }
        }
        ...
        if (!disableInstallation) {
            ...
            try {
                String packageName = getComponentProperties()
                        .get("packagename");
                String packageAsDir = packageName.replaceAll("\\.", "/");
                File dir = new File(sourceDirectory + "/" + packageAsDir);
                if (dir.exists()) {
                    Iterator filesIterator = FileUtils.iterateFiles(dir,
                            new String[] { "java" }, false);
                    while (filesIterator.hasNext()) {
                        File f = (File) filesIterator.next();
                        getLog().info(
                                "Copying generated '" + f.getName()
                                        + "' to project...");
                        FileUtils.copyFileToDirectory(f, new File(
                                destinationDirectory + "/src/main/java/"
                                        + packageAsDir));
                    }
                } else {
                    throw new MojoFailureException(
                            "No tables found in database to generate code from.");
                }
