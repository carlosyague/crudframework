...
/**
 * A base implementation of the {@link TreeWalker} interface. Uses a simple node
 * naming scheme of "0" for the root node, "0:0" for the first child, "0:1" for
...
public class TreeWalkerBase implements TreeWalker {
	...
	public boolean next() {
		if (!startedWalking) {
			// the first next() call just needs to set the root node and push it
			// onto the stack
		...
		if (prevNode.isLeaf()) {
			...
			if (prevNodeId.equals(tree.getNodeId())) {
				/**
				 * We know there is at least one child b/c otherwise we would
				 * have popped the node after checking isLeaf. Basically we need
				 * to keep drilling down until we reach the deepest node that is
				 * available for "walking." Then we'll return to the parent and
				 * render its siblings and walk back up the tree.
				 */
				...
				String parentNodeId = StringUtils.substringBeforeLast(
						currentNodeId, TREE_NODE_SEPARATOR);
