package org.librae.common.webapp.jsf;
...
public class PanelGridRenderer extends HtmlRenderer {
    ...
    public void encodeEnd(FacesContext facesContext, UIComponent component)
            throws IOException {
        RendererUtils
                .checkParamValidity(facesContext, component, UIPanel.class);

        int columns;
        if (component instanceof HtmlPanelGrid) {
            columns = ((HtmlPanelGrid) component).getColumns();
        } else {
            final Integer i = (Integer) component
                    .getAttributes()
                    .get(
                            org.apache.myfaces.shared_tomahawk.renderkit.JSFAttr.COLUMNS_ATTR);
            columns = i == null ? 0 : i.intValue();
        }
    ...
    protected void renderChildren(FacesContext context, ResponseWriter writer,
        ...
        if (component instanceof HtmlPanelGrid) {
            ...
            rowClasses = (String) component.getAttributes().get(
                    JSFAttr.ROW_CLASSES_ATTR);
        }

        final String[] rowClassesArray = (rowClasses == null) ? org.apache.myfaces.shared_tomahawk.util.ArrayUtils.EMPTY_STRING_ARRAY
                : StringUtils.trim(StringUtils
                        .splitShortString(rowClasses, ','));
        final int rowClassesCount = rowClassesArray.length;

        final int childCount = getChildCount(component);
        if (childCount > 0) {
            int columnIndex = 0;
            int rowClassIndex = 0;
            boolean rowStarted = false;
            for (final Iterator it = getChildren(component).iterator(); it
                    .hasNext();) {
