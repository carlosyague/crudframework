 ...
 * tree made up of instances of the {@link TreeNode} class.
 * 
 * @author Sean Schofield
 * @author Hans Bergsten (Some code taken from an example in his O'Reilly
 *         JavaServer Faces book. Copied with permission)
 * @version $Revision: 168205 $ $Date: 2005-05-04 18:59:21 -0400 (Wed, 04 May
 *          2005) $
 */
public class TreeModelBase implements TreeModel {
	...
	public String[] getPathInformation(String nodeId) {
		if (nodeId == null) {
			throw new IllegalArgumentException(
					"Cannot determine parents for a null node.");
		}
	...
	public boolean isLastChild(String nodeId) {
		...
		String childString = nodeId
				.substring(nodeId.lastIndexOf(SEPARATOR) + 1);
	...
	public TreeNode getNodeById(String nodeId) {
		...
		while (st.hasMoreTokens()) {
			...
			if (node == null) {
				...
				// don't worry about invalid index, that exception will be
				// caught later and dealt with
